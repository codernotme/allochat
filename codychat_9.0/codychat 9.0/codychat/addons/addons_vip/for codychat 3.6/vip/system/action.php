...
function vipAddUserPlan(){
	...
	$mysqli->query("UPDATE boom_users SET user_rank = 2, vip_end = '$new_time', user_action = user_action + 1 WHERE user_id = '{$user['user_id']}'");
	clearNotifyAction($user['user_id'], 'rank_change');
	postPrivate($data['system_id'], $user['user_id'], $message);
	boomNotify('rank_change', array('target'=> $user['user_id'], 'source'=> 'rank_change', 'rank'=> 2));
	...
	if(!empty($user2)){
		return boomTemplate('../addons/vip/system/template/vip_user', $user2);
	}
	else {
		return 0;
	}
...
function vipSearchUser(){
	...
	if($get_user->num_rows > 0){
		while($user = $get_user->fetch_assoc()){
			$list .= boomTemplate('../addons/vip/system/template/vip_user', $user);
		}
		return $list;
	}
	else {
...
if(isset($_POST['plan1'], $_POST['plan2'], $_POST['plan3'], $_POST['plan4'], $_POST['plan5'], $_POST['paypal_id'], $_POST['paypal_secret'], $_POST['currency'], $_POST['paypal_mode']) && boomAllow($cody['can_manage_addons'])){
	echo saveVipConfig();
	die();
}
if(isset($_POST['load_transaction']) && boomAllow($cody['can_manage_addons'])){
	echo vipLoadTransaction();
	die();
}
if(isset($_POST['vip_user'], $_POST['vip_plan'], $_POST['vip_add']) && boomAllow($cody['can_manage_addons'])){
	echo vipAddUserPlan();
	die();
}
if(isset($_POST['search_vip']) && boomAllow($cody['can_manage_addons'])){
	echo vipFindUserTransaction();
	die();
}
if(isset($_POST['vip_search_user']) && boomAllow($cody['can_manage_addons'])){
	echo vipSearchUser();
	die();
}
if(isset($_POST['vip_load_list']) && boomAllow($cody['can_manage_addons'])){
	echo vipLoadList();
	die();
}
if(isset($_POST['vip_cancel']) && boomALlow($cody['can_manage_addons'])){
	echo vipSingleClean();
	die();
}
?>