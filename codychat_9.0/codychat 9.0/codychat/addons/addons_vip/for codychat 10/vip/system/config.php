	...
	<div class="page_element">
		<div id="vip">
			<div id="vip_setting" class="tab_zone">
				...
				<button id="save_vip_plan" onclick="saveVip();" type="button" class="tmargin10 reg_button theme_btn"><?php echo $lang['save']; ?></button>
			</div>
				...
				vipConfirmCancel = function(vip){
					$.post('addons/vip/system/action.php', {
						...
						token: utk,
							...
							if(response == 1){
								hideModal();
								callSuccess(system.action_complete, 1);
					...
					if(vipWait == 0){
						...
						$.post('addons/vip/system/action.php', {
							...
							token: utk,
								...
								vipWait = 0;
								if(response == 2){
									callError(system.no_user);
								}
								else if(response == 3){
									callError(system.cannot_user);
								}
