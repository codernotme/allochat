	...
	if( $search_answer->num_rows > 0){
		if($search_answer->num_rows > 0){
			while($ra = $search_answer->fetch_array()){
				$list_bot .= boomTemplate('../addons/superbot/system/template/superbot_data', $ra);			
			}
		}
		return $list_bot;
	}
...
if(isset($_POST['question'], $_POST['answer1'], $_POST['answer2'], $_POST['answer3'], $_POST['answer4'], $_POST['answer5'])){
	...
	$sbdelete = escape($_POST['sbdelete']);
	$mysqli->query("DELETE FROM superbot_data WHERE id = '$sbdelete'");
	echo 2452432;
}
else if(isset($_POST['set_superbot_access']) && boomAllow($cody['can_manage_addons'])){
	$superbot_access = escape($_POST['set_superbot_access']);
